...
public class Grid : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                Ray ray = new Ray(worldPoint + Vector3.up * 50, Vector3.down);//get worldpoint, move up 50, and look down
                RaycastHit hit;
                if (Physics.Raycast(ray, out hit, 100, walkableMask))//if get a hit
                {
                    Debug.Log(hit.point);
                    if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Jelly"))
                    {
                        Debug.Log("Jelly Time");
                    }
    ...
    void OnDrawGizmos()
    {
        ...
        //testing for raycast
        Gizmos.DrawWireCube(new Vector3(0,0,0)+Vector3.up * 50, new Vector3(5, 5, 5));
        Debug.DrawRay(new Vector3(0, 0, 0) + Vector3.up * 50, Vector3.down * 100);
    }
